...
export default function AdCard ({ img ,name, location, price, rating, edit=false }) {
  ...
  return (
    <div className={`bg-white rounded-md border-solid border-2 p-4 max-w-md md:max-w-xl cursor-pointer ${event_card} ${edit?"max-h-md":"max-h-md md:max-h-80 md:h-64"}`}>
      ...
      <div className="flex flex-row justify-between items-center rounded pt-2 px-1">
        ...
        {
          edit ? (<div className="flex flex-col pr-1">
          <Link href="/app/announcement/edit">
            <span className={`${text_size["m_text"]} flex flex-row justify-between hover:underline`}>
              Editar
